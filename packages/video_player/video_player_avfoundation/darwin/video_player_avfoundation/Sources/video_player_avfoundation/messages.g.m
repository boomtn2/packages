  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FVPAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(forPlayer:bandwidth:error:)], @"FVPAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(forPlayer:bandwidth:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSInteger arg_textureId = [GetNullableObjectAtIndex(args, 0) integerValue];
        double arg_mixWithOthers = [GetNullableObjectAtIndex(args, 1) doubleValue];
        FlutterError *error;
        [api forPlayer:arg_textureId bandwidth:arg_mixWithOthers error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
  ...
  }
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        NSInteger arg_textureId = [GetNullableObjectAtIndex(args, 0) integerValue];
        FlutterError *error;
        NSArray<FVPVideoResolution *> *output = [api forPlayer:arg_textureId error:&error];
        callback(wrapResult(output, error));
      }];
    } else {
      [channel setMessageHandler:nil];
